...
## Prompt 48: 100% Dynamic LLM Response Generation & Retry Policy Query
```
every resonse of interviwer must be generated by llm , no offline fallback okay/ each asnwer must be diffrent
```

One-line summary: User strictly instructing that 100% of interviewer responses must be dynamically generated by Groq LLM with retries, eliminating static offline templates to guarantee every turn is completely unique.

---

## Prompt 49: Mandatory Genuine PDF Export Alignment Query
```
it syas pdf but doenloads md file, seems issue on backend , make it a downloadable pdf, ( u can add prompts now)
```
